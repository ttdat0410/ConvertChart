...
internal class GradientDrawingLayer : ScrollableGraphViewDrawingLayer {
    ...
    override func drawInContext(ctx: CGContext) {
        ...
        let gradient = CGGradientCreateWithColors(colorSpace, colors as CFArray, locations)//CGGradient(colorSpace, colors: colors as CFArray, locations: locations)
        ...
        switch(gradientType) {
        case .linear:
//            ctx.CGContextDrawLinearGradient(gradient!, start: topCentre, end: bottomCentre, options: .drawsAfterEndLocation)
            CGContextDrawLinearGradient(ctx, gradient, topCentre, bottomCentre, .DrawsAfterEndLocation)

        case .radial:
//            ctx.CGContextDrawLinearGradient(gradient!, startCenter: topCentre, startRadius: startRadius, endCenter: topCentre, endRadius: endRadius, options: .drawsAfterEndLocation)
