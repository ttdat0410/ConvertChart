...
class Plot {
    ...
    private func dequeue(animation: GraphPointAnimation) {
        if let index = currentAnimations.indexOf(animation) {
            currentAnimations.removeAtIndex(index)//remove(at: index)
        }
    ...
    internal func startAnimations(forPoints pointsToAnimate: Range<Int>, withData data: [Double], withStaggerValue stagger: Double) {
    ...
    internal func setPlotPointPositions(forNewlyActivatedPoints newPoints: Range<Int>, withData data: [Double]) {
    ...
    internal func animatePlotPointPositions(forPoints pointsToAnimate: Range<Int>, withData data: [Double], withDelay delay: Double) {
        ...
        for pointIndex in pointsToAnimate {
            ...
            animate(point, to: newPosition, withDelay: Double(dataIndex) * delay)
            dataIndex += 1
        }
    }
