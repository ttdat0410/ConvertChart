...
@objc class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    var backgroundFillColor: UIColor = UIColor.whiteColor()
    ...
    var shouldRangeAlwaysStartAtZero: Bool = false
    ...
    var shouldAdaptRange: Bool = false
    ...
    override func prepareForInterfaceBuilder() {
        ...
        self.addReferenceLines(referenceLines)
    }
    ...
    private func setup() {
        ...
        while(queuedPlots.count > 0) {
            if let plot = queuedPlots.dequeue() {
                addPlotToGraph(plot, activePointsInterval: initialActivePointsInterval)
            }
        }
    ...
    private func addDrawingLayersForPlots(inViewport viewport: CGRect) {
        for plot in plots {
            addSubLayers(plot.layers(forViewport: viewport))
        }
    }
    ...
    private func updateOffsetWidths() {
        ...
        updateOffsetsForGradients(offsetWidth)
    ...
    private func updateFrames() {
        ...
        updateFramesForGradientLayers(viewportWidth, viewportHeight: viewportHeight)
    ...
    func addReferenceLines(referenceLines: ReferenceLines) {
        ...
        // Otherwise, add the reference lines, reload everything.
            ...
            addReferenceLinesToGraph(referenceLines)
        }
    }
    ...
    private func addPlotToGraph(plot: Plot, activePointsInterval: Range<Int>) {
        plot.graphViewDrawingDelegate = self
        self.plots.append(plot)
        initPlot(plot, activePointsInterval: activePointsInterval)
        startAnimations(withStaggerValue: 0.15)
    }
    ...
    private func initPlot(plot: Plot, activePointsInterval: Range<Int>) {
        
        #if !TARGET_INTERFACE_BUILDER
            plot.setup() // Only init the animations for plots if we are not in IB
        #endif
        
        plot.createPlotPoints(dataSource!.numberOfPoints(), range: range) // TODO: removed forced unwrap
        ...
        addSubLayers(plot.layers(forViewport: currentViewport()))
    }

    private var queuedPlots: SGVQueue<Plot> = SGVQueue<Plot>()
    
    private func enqueuePlot(plot: Plot) {
        queuedPlots.enqueue(plot)
    }
    ...
    private func calculateActivePointsInterval() -> Range<Int> {
        ...
        return actualMin..<actualMax.advancedBy(1)//advanced(by: 1)
    }
    
    // Calculate the range across all plots.
    private func calculateRange(forActivePointsInterval interval: Range<Int>) -> (min: Double, max: Double) {
    ...
    private func calculateRange(forPlot plot: Plot, forActivePointsInterval interval: Range<Int>) -> (min: Double, max: Double) {
...
//    }
    ...
    private func getData(forPlot plot: Plot, andActiveInterval activeInterval: Range<Int>) -> [Double] {
    ...
    private func determineDeactivatedPoints() -> [Int] {
        ...
        let deactivatedPoints = prevSet.subtract(currSet)//subtracting(currSet)
    ...
    private func updateLabelsForCurrentInterval() {
        ...
        if let ref = self.referenceLines {
            if(ref.shouldShowLabels) {
                ...
                updateLabels(filteredPoints, activatedPoints: filteredPoints)
            }
        }
    }
    ...
    internal func intervalForActivePoints() -> Range<Int> {
